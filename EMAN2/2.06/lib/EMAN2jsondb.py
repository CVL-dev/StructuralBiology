...
def jdb_open_dict(url,ro=False):
	"""Opens a JSON file as a dictionary-like object"""

	return JDBDict.open_dict(url,ro)

def db_close_dict(url):
	"""Closes a named dictionary to free resources. Note that actual JDB files are only open when reading or writing.
	Filesystem locking mechanisms are used to avoid conflicts. Closing the dict will free up the copy in memory, but will
	not invalidate references to the open dict. If an existing reference is reused, the dict will be re-read from disk."""

	JDBDict.free_dict(url,ro)
	return

def db_remove_dict(url):
	"""closes and deletes a database using the same specification as db_open_dict. This will effectively invalidate any
	other open references to the dict."""
