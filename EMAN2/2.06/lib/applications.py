		...
		Iter = 0
 		while(Iter < max_iter and terminate == 0):
			...
			for im in xrange( nima ):
				...
				peak2 = None
				if ( len(refrings1) > 0):
				        print "Only consider C1 symmetry for now"
				        sys.exit()
					#peak1, phihi1, theta1, psi1, sxi1, syi1, t11 = proj_ali_helical_90_local(data[im],refrings1,numr,xrng[N_step],yrng[N_step],stepx[N_step],ynumber[N_step], an[N_step], psi_max,  finfo,)
				if( len(refrings2) > 0):
					peak2, phihi2, theta2, psi2, sxi2, syi2, t12 = proj_ali_helical_local(data[im],refrings2,numr,xrng[N_step],yrng[N_step],stepx[N_step],ynumber[N_step], an[N_step], psi_max,finfo,sym_string=symmetry_string,yrnglocal=yrnglocal,CONS=True)
				if peak1 is None: 
					peak = peak2
					phihi = phihi2
					theta = theta2
