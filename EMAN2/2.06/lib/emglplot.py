...
class EMGLPlot(EMImage3DGUIModule):
	...
	def cylinder_to_from(self,next,prev,val=0.5):
		dx = next[0] - prev[0]
		dy = next[1] - prev[1]
		dz = next[2] - prev[2]
	...
	def unitcircle(self):
		...
		for i in range(0,int(n)):
			...
			p1 = [a,b,0.]
			p2 = [c,d,0.]
			self.cylinder_to_from(p2,p1,0.3)
	def render(self):
		...
		for i in range(0,n-1):
			...
			self.cylinder_to_from(p2,p1,0.1)
